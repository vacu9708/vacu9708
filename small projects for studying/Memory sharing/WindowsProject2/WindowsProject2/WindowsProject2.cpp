...
DWORD WINAPI ball_receiver(PVOID pArg) {
	Sleep(100);
	while (1) {
		if (ball_information->ball_sent_to_2) { // When ball information has arrived
			y = ball_information->y;
			xv = ball_information->xv;
			yv = ball_information->yv;
			ball_information->ball_sent_to_2 = false;
			without_ball = 0;
		}
	}
}
...
void moveBall(){
	...
	if (x < crt.left - R) {// When the ball got to the left
		...
		ball_information->ball_sent_to_1 = true;
	}
	else if (x > crt.right + R) { // Bounce if the ball has gotten to the right
		without_ball = 1;
		ball_information->y = y;
		ball_information->xv = xv;
		ball_information->yv = yv;
		ball_information->ball_sent_to_3 = true;
	}
