...
DWORD WINAPI ball_receiver(PVOID pArg) {
	Sleep(100);
	while (1) {
		if (ball_information->ball_sent_to_3) { // When ball information has arrived
			y = ball_information->y;
			xv = ball_information->xv;
			yv = ball_information->yv;
			ball_information->ball_sent_to_3 = false;
			without_ball = 0;
		}
	}
}
...
void moveStick()
{
	...
	if (sY[1] <= 0 || sY[1] >= crt.bottom - H) // Right stick
	{
		sYv[1] *= -1;
	}
	sY[1] += sYv[1];
...
{
	if (x >= crt.right - R - W && y > sY[1] - R && y < sY[1] + H + R) // Right stick
	{
		x -= R / 2;
		xv *= (-BOUNCE);
	}
